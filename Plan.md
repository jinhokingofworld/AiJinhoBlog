...
이 프로젝트의 목표는 기본 블로그 기능 위에 RAG, MCP, AI Agent 기능을 단계적으로 결합하여, 사용자의 과거 글을 개인 지식 기반으로 활용하는 AI 블로그 플랫폼을 완성하는 것이다.
...
#### Main Issue: 과거 글 기반 검색 및 답변

1. Medium Issue: 유사 게시글 검색
   - Detailed Issue: 제목 기반 유사 게시글 검색 API 구현
   - Detailed Issue: 본문 기반 유사 게시글 검색 API 구현
   - Detailed Issue: 유사도 점수와 근거 게시글 반환

2. Medium Issue: 내 기억 Q&A
   - Detailed Issue: 질문 임베딩 생성
   - Detailed Issue: 관련 chunk 검색
   - Detailed Issue: 검색 결과를 포함한 답변 생성
   - Detailed Issue: 답변에 참고 게시글 링크 표시

3. Medium Issue: 중복 작성 방지 알림
   - Detailed Issue: 게시글 발행 직전 유사도 검사
   - Detailed Issue: 중복 가능성이 높은 게시글 목록 표시
   - Detailed Issue: 사용자가 무시하고 발행할 수 있는 흐름 구현

완료 기준:

- 사용자가 자연어 질문으로 과거 글 기반 답변을 받을 수 있다.
- 답변에는 어떤 게시글을 근거로 삼았는지 표시된다.
- 새 글 발행 전 유사 글이 있으면 알림이 표시된다.

### Phase 4. MCP 연동

#### Main Issue: 외부 MCP 연동

1. Medium Issue: Dropbox MCP 연결
   - Detailed Issue: Dropbox MCP 인증 방식 정리
   - Detailed Issue: 문서 목록 조회 기능 구현
   - Detailed Issue: 문서 내용 검색 기능 구현
   - Detailed Issue: 검색 결과를 블로그 작성 흐름에 연결

완료 기준:

- 사용자가 Dropbox 문서를 검색하고, 검색 결과를 글 작성 참고 자료로 사용할 수 있다.
...
8. RAG 기반 유사 게시글 검색 구현
9. 내 기억 Q&A 구현
10. 중복 작성 방지 알림 구현
11. Dropbox MCP 연동
12. 자체 MCP 서버와 게시글 CRUD tool 구현
13. 링크 및 이미지 분석 저장 구현
14. 글감 추천 Agent 구현
15. 개인 문체 변환 구현
16. 출판 퀄리티 리팩토링 및 Before/After 비교 구현
17. 테스트, 보안, 성능 점검
18. 배포 문서화 및 운영 환경 구성
